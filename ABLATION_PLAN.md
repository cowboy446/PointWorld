...
- robot 是否使用 `gripper_open` feature。
...
- `--robot_use_gripper_open_feature=true|false`
...
- `pointworld/base.py` 根据实际启用的 robot feature names 切片 `robot_features` 和 norm stats，再建立对应输入维度的 `robot_proj`。
